...
exports.run = async (client, message, args) => {
    ...
    let emojilisten = message.guild.emojis.cache.get(emoji => emoji.name === emojiname),
      rname = message.content.split(",").slice(1, 2).join(" "),
      embedmessage = message.content.split(",").slice(2).join(" "),
      role = message.guild.roles.cache.find(val => val.name === rname);
    ...
    let botRolePosition = message.guild.member(client.user).roles.highest.position,
      ...
      userRolePossition = message.member.roles.highest.position;
